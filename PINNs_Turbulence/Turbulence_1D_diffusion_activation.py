...
class PhysicsInformedNN:
    ...
    def __init__(self, t, x, u,
        ...
        # loss
        self.loss_u = tf.reduce_sum(tf.square(self.u_tf - self.u_pred))
        self.loss_eq = tf.reduce_sum(tf.square(self.eq_pred))
                
        # optimizers
        self.optimizer = tf.train.AdamOptimizer(learning_rate = self.learning_rate)
        
        self.train_op_u = self.optimizer.minimize(self.loss_u)
        self.train_op_eq = self.optimizer.minimize(self.loss_eq)
    ...
    def train(self, num_epochs, batch_size, learning_rate):
        ...
        for epoch in range(num_epochs):
            ...
            for it in range(0, N, batch_size):
                ...
                self.sess.run([self.train_op_u,
                               self.train_op_eq], tf_dict)
                ...
                if it % (10*batch_size) == 0:
                    ...
                    [loss_u_value,
                     loss_eq_value,
                     learning_rate_value] = self.sess.run([self.loss_u,
                                                           self.loss_eq,
                                                           self.learning_rate], tf_dict)
                    print('Epoch: %d, It: %d, Loss u: %.3e, Loss eq: %.3e, Time: %.2f, Learning Rate: %.3e' % 
                          (epoch, it, loss_u_value, loss_eq_value, elapsed, learning_rate_value))
                    start_time = time.time()
