...
class PhysicsInformedNN:
    ...
    def __init__(self, t, x, u, d,
        ...
        # initialize NN
        self.weights_1_ud, self.biases_1_ud, self.weights_2_ud, self.biases_2_ud = self.initialize_NN(layers_ud)
    ...
    def initialize_NN(self, layers):
        ...
        for l in range(0,num_layers-1):
            W1 = self.xavier_init(size=[layers[l], layers[l+1]]) # P x Q , Q x Q, Q x R
            b1 = tf.Variable(tf.zeros([1,layers[l+1]], dtype=tf.float32), dtype=tf.float32) # 1 x Q, 1 x Q, 1 x R
            W2 = self.xavier_init(size=[layers[l+1], layers[l+1]]) # Q x Q, Q x Q, R x R
            b2 = tf.Variable(tf.zeros([1,layers[l+1]], dtype=tf.float32), dtype=tf.float32) # 1 x Q, 1 x Q, 1 x R
            weights_1.append(W1)
            biases_1.append(b1)
            weights_2.append(W2)
            biases_2.append(b2)
    ...
    def neural_net(self, X, weights_1, biases_1, weights_2, biases_2):
        num_layers = len(weights_1) + 1
        
        W1 = weights_1[0]
        b1 = biases_1[0]        
        H = tf.matmul(X, W1) # N x P --> N x Q
        ...
        for l in range(1,num_layers-2):
            ...
            W2 = weights_2[l]
            b2 = biases_2[l]
            H = H + tf.matmul(tf.tanh(tf.matmul(H, W1) + b1),W2) + b2  # N x Q --> N x Q
        
        W1 = weights_1[-1]
        b1 = biases_1[-1]
        Y = tf.matmul(H, W1) # N x Q --> N x R
    ...
    def net_u(self, t, x):
        ...
        ud = self.neural_net(tx, self.weights_1_ud, self.biases_1_ud, self.weights_2_ud, self.biases_2_ud)
...
#    ax.axis('tight')
    ...
    model.train(num_epochs = 1*10**5, batch_size = 20000, learning_rate=1e-3)
    model.train(num_epochs = 2*10**5, batch_size = 20000, learning_rate=1e-4)
    model.train(num_epochs = 3*10**5, batch_size = 20000, learning_rate=1e-5)
    model.train(num_epochs = 4*10**5, batch_size = 20000, learning_rate=1e-6)
...
#    print(model.sess.run(loss_eq, tf_dict))
    ...
    scipy.io.savemat('turbulence_1D_diffusion_results_%s.mat' %(time.strftime('%d_%m_%Y')),
