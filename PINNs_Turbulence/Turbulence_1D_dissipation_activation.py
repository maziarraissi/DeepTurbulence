...
class PhysicsInformedNN:
    ...
    def __init__(self, t, x, u, e,
        ...
        self.beta = tf.Variable(tf.zeros([1], dtype=tf.float32), dtype=tf.float32)
        ...
        self.loss = tf.reduce_sum(tf.square((self.u_tf - self.u_pred)/self.ue_std[0])) + \
                    self.beta*tf.reduce_sum(tf.square(self.eq_pred/self.ue_std[0]))
        
        self.loss_mean = tf.reduce_mean(tf.square((self.u_tf - self.u_pred)/self.ue_std[0])) + \
                         self.beta*tf.reduce_mean(tf.square(self.eq_pred/self.ue_std[0]))
        
        # optimizers
        self.optimizer = tf.train.AdamOptimizer(learning_rate = self.learning_rate)
        self.train_op_min = self.optimizer.minimize(self.loss, var_list = self.weights_ue + self.biases_ue)
        self.train_op_max = self.optimizer.minimize(-self.loss, var_list = self.beta)
    ...
    def train(self, num_epochs, batch_size, learning_rate):
        ...
        for epoch in range(num_epochs):
            ...
            for it in range(0, N, batch_size):
                ...
                self.sess.run(self.train_op_min, tf_dict)
                self.sess.run(self.train_op_max, tf_dict)
                
                # Print
                if it % (10*batch_size) == 0:
                    elapsed = time.time() - start_time
                    loss_value, learning_rate_value, beta_value = self.sess.run([self.loss_mean, self.learning_rate, self.beta], tf_dict)
                    print('Epoch: %d, It: %d, Loss: %.3e, Time: %.2f, Learning Rate: %.3e, beta: %.2f' % 
                          (epoch, it, loss_value, elapsed, learning_rate_value, beta_value))
                    start_time = time.time()
...
#    ax.axis('tight')
    ...
    model.train(num_epochs = 2*10**5, batch_size = 20000, learning_rate=1e-3)
    model.train(num_epochs = 3*10**5, batch_size = 20000, learning_rate=1e-4)
    model.train(num_epochs = 3*10**5, batch_size = 20000, learning_rate=1e-5)
    model.train(num_epochs = 2*10**5, batch_size = 20000, learning_rate=1e-6)
...
#    print(model.sess.run(loss_eq, tf_dict))
    ...
    scipy.io.savemat('turbulence_1D_dissipation_results_%s.mat' %(time.strftime('%d_%m_%Y')),
